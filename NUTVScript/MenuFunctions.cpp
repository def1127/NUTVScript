...
void getUserInput() {
	...
	std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); //clear the buffer of anything left over
}
...
void ls() { //print out all files in the active directory
	...
	for (const std::string path = "/Program Files/NUTVScript"; const auto& file : std::filesystem::directory_iterator(path))
		std::cout << file.path() << std::endl;
}

void man(const std::string& command) { //print out detailed help information about each command as requested
	std::cout << std::endl;
